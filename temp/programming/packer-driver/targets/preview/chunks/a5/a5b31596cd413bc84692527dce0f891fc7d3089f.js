    ...
    execute: function () {
      ...
      _export("GameUtil", GameUtil = class GameUtil {
        ...
        static formatWeight(weight) {
          ...
          var inTons = Math.floor(weight / 1000 * 100) / 100;
          return inTons + "T";
        }
        /**
        ...
        static hexToRGBA(hex, alpha) {
          ...
          var b = parseInt(hex.substring(4, 6), 16); // 返回 RGBA 颜色对象
        ...
        static getWorldPositionAsVec2(node) {
          var worldPosition = node.getWorldPosition().clone(); // 获取世界坐标
        ...
        */
        ...
        static calculateRayEnd(fromNode, rayLength) {
          var rotation = fromNode.angle; // 根据角度计算方向向量
          ...
          if (rotation === -90) {
            ...
            var adjustedAngle = rotation - 90;
            direction = v2(-Math.cos(adjustedAngle * (Math.PI / 180)), Math.sin(-adjustedAngle * (Math.PI / 180)));
          } // 计算射线起点坐标


          var objs = this.getWorldPositionAsVec2(fromNode);
          var obje = objs.add(direction.multiplyScalar(rayLength));
          return obje;
        }
        /**
         * 判断两个 AABB 是否相交
        ...
        static isCircleIntersectingLine(circleCenter, radius, lineStart, lineEnd) {
          ...
          var toCircle = circleCenter.subtract(lineStart);
          var projection = toCircle.dot(lineDir.normalize());
          var closestPoint = lineStart.add(lineDir.normalize().multiplyScalar(projection));
          var distance = circleCenter.subtract(closestPoint).length();
        ...
        static getCircleAABB(circleCollider) {
          ...
          var center = circleCollider.worldPosition;
        ...
        static getPolygonAABB(polygonCollider) {
          var points = polygonCollider.worldPoints;
          var minX = points[0].x;
          var maxX = points[0].x;
          var minY = points[0].y;
          var maxY = points[0].y;
        ...
        static isPolygonAndCircleIntersecting(polygonCollider, circleCollider) {
          ...
          // }
          // 2. 精确检测
          var circleCenter = new Vec2(circleCollider.node.worldPosition.x, circleCollider.node.worldPosition.y); // 圆心

          var radius = circleCollider.radius; // 检查圆心是否在多边形内部
        ...
        static worldToScreenLocal(target, localNode) {
          var targetWorldPos = target.worldPosition.clone();
          var mainCamera = (_crd && StormSunderGlobalInstance === void 0 ? (_reportPossibleCrUseOfStormSunderGlobalInstance({
            error: Error()
          }), StormSunderGlobalInstance) : StormSunderGlobalInstance).instance.camera;
          var screenPos = mainCamera.worldToScreen(targetWorldPos);
          var uiPos = new Vec3(); // localNode.getComponent(UITransform)!.convertToNodeSpaceAR(
