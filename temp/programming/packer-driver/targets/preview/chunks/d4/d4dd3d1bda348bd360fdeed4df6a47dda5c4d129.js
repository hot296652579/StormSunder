    ...
    execute: function () {
      ...
      _export("AttributeBonusMgr", AttributeBonusMgr = class AttributeBonusMgr {
        ...
        checkMoneyEnough(cost) {
          var userMoney = this.userModel.money;
        ...
        updateBonus(type) {
          var bonus = this.userModel.bonusData[type];
          if (!bonus) return; // 更新等级和升级消耗

          this.userModel.bonusData[type] = _extends({}, bonus, {
            level: bonus.level,
        ...
        getUpgradeCost(type) {
          var config = this.developConfig.getConfigById(type);
          if (!config) return 0;
        ...
        getBonus(type, isAI) {
          ...
          if (!isAI) {
            ...
            var aiRange = this.userModel.game_ai_range;
            var randomOffset = Math.floor(Math.random() * (aiRange[1] - aiRange[0] + 1)) + aiRange[0];
            var newLv = level + randomOffset;
        ...
        getStormSunderAttack(lv, isAI) {
          var external_attack = this.getBonus(BonusType.ATTACK, isAI);
          var game_attack_base = this.userModel.game_attack_base;
          var game_lv_attack_up = this.userModel.game_lv_attack_up;
        ...
        getStormSunderSpeed(lv, isAI) {
          var external_speed = this.getBonus(BonusType.SPEED, isAI);
          var game_speed_base = this.userModel.game_speed_base;
          var game_lv_speed_up = this.userModel.game_lv_speed_up;
        ...
        getStormSunderExp(lv) {
          var game_absorbec_exp_base = this.userModel.game_absorbec_exp_base;
          var game_lv_absorbec_exp_up = this.userModel.game_lv_absorbec_exp_up;
        ...
        getExpBonus(exp, isAI) {
          var expProgress = Math.floor(this.getBonus(BonusType.EXP, isAI) / 100);
