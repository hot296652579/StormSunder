  ...
  return {
    ...
    execute: function () {
      ...
      _export("TornadoAIComponent", TornadoAIComponent = (_dec = ccclass('TornadoAIComponent'), _dec(_class = class TornadoAIComponent extends (_crd && TornadoComponent === void 0 ? (_reportPossibleCrUseOfTornadoComponent({
        ...
        initAIPlayer() {
          const aiConfig = (_crd && PlayerMgr === void 0 ? (_reportPossibleCrUseOfPlayerMgr({
            error: Error()
        ...
        decideAction() {
          ...
          const move = Math.random() * 100 < this.moveProbability; // console.log(`AI 行为判断:移动概率:${this.moveProbability} 是否移动:${move}`);

          if (move) {
            this.randomMove();
          } else {
            const closestItem = (_crd && PropMgr === void 0 ? (_reportPossibleCrUseOfPropMgr({
              error: Error()
        ...
        randomMove() {
          ...
          const maxDistance = this.speed * this.moveDuration; // 随机方向的单位向量
        ...
        onTriggerStay(event) {
          ...
          if (otherCollider.getGroup() == 1 << 3) {
            const targetTornado = otherCollider.node.parent.getComponent(_crd && TornadoComponent === void 0 ? (_reportPossibleCrUseOfTornadoComponent({
              error: Error()
            }), TornadoComponent) : TornadoComponent);
            if (!targetTornado) return;
            const isPlayer = targetTornado instanceof (_crd && TornadoComponent === void 0 ? (_reportPossibleCrUseOfTornadoComponent({
              error: Error()
            }), TornadoComponent) : TornadoComponent);
        ...
        onRadiusTriggerEnter(event) {
          ...
          if (event.otherCollider.getGroup() == 1 << 3) {
            const targetTornado = otherNode.parent.getComponent(_crd && TornadoComponent === void 0 ? (_reportPossibleCrUseOfTornadoComponent({
              error: Error()
            }), TornadoComponent) : TornadoComponent);
            if (!targetTornado) return;
        ...
        escapeFrom(target) {
          ...
          const maxDistance = this.speed * this.escapeDuration; // 计算逃离方向，指向目标的反方向
