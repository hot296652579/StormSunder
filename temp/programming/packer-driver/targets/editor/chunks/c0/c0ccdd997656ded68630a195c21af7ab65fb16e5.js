    ...
    execute: function () {
      ...
      _export("PropMgr", PropMgr = class PropMgr {
        ...
        async genatorProp() {
          ...
          const propsUI = (_crd && StormSunderGlobalInstance === void 0 ? (_reportPossibleCrUseOfStormSunderGlobalInstance({
            error: Error()
          }), StormSunderGlobalInstance) : StormSunderGlobalInstance).instance.props;
          this.tornadoNode = await (_crd && PlayerMgr === void 0 ? (_reportPossibleCrUseOfPlayerMgr({
            error: Error()
          }), PlayerMgr) : PlayerMgr).inst.getTornadoNode();

          for (let index = 0; index < this.propInitNum; index++) {
            let spawnPos = this.getValidSpawnPosition(); // 进行四向物理检测

            if (spawnPos && !this.isPositionBlocked(spawnPos)) {
              const propPrefab = await (_crd && resLoader === void 0 ? (_reportPossibleCrUseOfresLoader({
                error: Error()
              ...
              let newMonster = instantiate(propPrefab);
              newMonster.setParent(propsUI);
              newMonster.setWorldPosition(spawnPos);
              this.curMovePropsCount++; // console.log("怪物生成成功", spawnPos);
        ...
        getRandomSpawnPosition() {
          ...
          let distance = Math.random() * this.spawnRadius + this.spawnRadius; // 创建射线检测前方障碍物

          let ray = new geometry.Ray(playerPos.x, playerPos.y, playerPos.z, Math.cos(angle), 0, Math.sin(angle));
        ...
        getNearestProp(playerNode) {
          ...
          const propsNodes = (_crd && StormSunderGlobalInstance === void 0 ? (_reportPossibleCrUseOfStormSunderGlobalInstance({
            error: Error()
          }), StormSunderGlobalInstance) : StormSunderGlobalInstance).instance.props;
