    ...
    execute: function () {
      ...
      _export("PropComponent", PropComponent = (_dec = ccclass('PropComponent'), _dec2 = property({
        ...
        async hurt(damage) {
          ...
          this.currentHp -= damage; // console.log(`当前道具血量:${this.currentHp}`);

          if (this.currentHp > 0) {
            const hpPercent = this.currentHp / this.hp; // 显示或更新血条

            await (_crd && Effect2DUIMgr === void 0 ? (_reportPossibleCrUseOfEffect2DUIMgr({
              error: Error()
            }), Effect2DUIMgr) : Effect2DUIMgr).Instance.showBlood(this.node, hpPercent);
          } //小于0时死亡
