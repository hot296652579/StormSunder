    ...
    execute: function () {
      ...
      _export("TornadoComponent", TornadoComponent = (_dec = ccclass('TornadoComponent'), _dec(_class = class TornadoComponent extends Component {
        ...
        setPositionByObstacle(event, cb) {
          ...
          const newPosition = _originalPosition.add(reverseDirection); // 直接修改坐标
        ...
        calculateReverseDirection(event) {
          ...
          const selfPos = this.node.worldPosition; // 计算方向向量（从障碍物指向自己）

          const direction = new Vec3(selfPos.x - obstaclePos.x, 0, // Y轴保持0
        ...
        onTriggerStay(event) {
          ...
          if (otherCollider.getGroup() == 1 << 4) {
            ...
            const propComponent = this.curHitObj.getComponent(_crd && PropComponent === void 0 ? (_reportPossibleCrUseOfPropComponent({
              error: Error()
            }), PropComponent) : PropComponent);
            const currentTime = game.totalTime;
            const nodeId = otherCollider.node.uuid;
            const lastTime = this._lastAttackTime.get(nodeId) || 0; // 检查是否达到攻击间隔
        ...
        onMoveHandler() {
          ...
          const playerDir = (_crd && UIJoyStick === void 0 ? (_reportPossibleCrUseOfUIJoyStick({
            error: Error()
        ...
        addExpByKill(isAI) {
          ...
          const propComp = this.curHitObj.getComponent(_crd && PropComponent === void 0 ? (_reportPossibleCrUseOfPropComponent({
            error: Error()
          }), PropComponent) : PropComponent);
          let objExp = 0;
          ...
          const finialExp = (_crd && AttributeBonusMgr === void 0 ? (_reportPossibleCrUseOfAttributeBonusMgr({
            error: Error()
        ...
        stormLevelUp() {
          const attributeBonusMgr = (_crd && AttributeBonusMgr === void 0 ? (_reportPossibleCrUseOfAttributeBonusMgr({
            error: Error()
          }), AttributeBonusMgr) : AttributeBonusMgr).inst;
